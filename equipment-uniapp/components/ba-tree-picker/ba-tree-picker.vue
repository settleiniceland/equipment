					...
					<block v-for="(item, index) in treeList" :key="index">
						<view class="tree-item" :style="[{
							...
							<view class="item-label">
								...
								<view class="uni-flex-item uni-inline-item" style="display: flex;" @tap.stop="_onItemSelect(item, index)">
	...
	export default {
		...
		methods: {
			_show(item,tree) {
				...
				this.customAttribute = item;
				this.localdata = tree;
			},
			_hide() {
				this.showDialog = false;
				this.customAttribute = undefined;
				// this._initTree();
			},
			_cancel() {
				this._hide()
			...
			_confirm() { //多选
				...
				this.$emit("select-change", selectedList, this.customAttribute);
				this._hide()
			},
			//格式化原数据（原数据为tree结构）
			_formatTreeData(list = [], level = 0, parentItem, isShowChild = true) {
				...
				list.forEach(item => {
					...
					let itemT = {
						...
						name: item[this.textKey1] + ' ' + item[this.textKey2],
						equipName: item[this.textKey2],
						code: item[this.textKey1],
						attribute: item[this.textKey3],
						level,
						isLastLevel,
						isShow: isShowChild,
						isShowChild: false,
			...
			_onItemSelect(item, index) {
				if (!this.multiple) { //单选
					...
					this.$emit("select-change", selected, this.customAttribute);
					this._hide();
					// this.treeList.forEach((v, i) => {
					// 	this.treeList[i].checkStatus = 0
					// })
