<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['system:notify-template:create']">{{ $t("message.Button.add") }}</el-button>
      </el-col>
      ...
      <el-table-column :label='$t("message.Button.operation")' align="center" class-name="small-padding fixed-width" width="150">
        <template v-slot="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['system:notify-template:update']">{{ $t("message.Button.update") }}</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['system:notify-template:delete']">{{ $t("message.Button.del") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog title="发送站内信" :visible.sync="sendNotifyOpen" width="500px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitSendNotifyForm">{{ $t("message.Button.determine") }}</el-button>
        <el-button @click="cancelSendNotify">{{ $t("message.Button.cancel") }}</el-button>
      </div>
    </el-dialog>
