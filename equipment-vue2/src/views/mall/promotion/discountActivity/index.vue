    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['promotion:discount-activity:create']">{{ $t("message.Button.add") }}</el-button>
      </el-col>
      ...
      <el-table-column :label='$t("message.Button.operation")' align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     ...
                     v-hasPermi="['promotion:discount-activity:update']">{{ $t("message.Button.update") }}</el-button>
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-if="scope.row.status === PromotionActivityStatusEnum.CLOSE.type"
                     v-hasPermi="['promotion:discount-activity:delete']">{{ $t("message.Button.del") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="商品选择">
          ...
          <el-table v-loading="loading" :data="form.products">
            ...
            <el-table-column :label='$t("message.Button.operation")' align="center" class-name="small-padding fixed-width">
              <template v-slot="scope">
                <el-button size="mini" type="text" icon="el-icon-delete" @click="removeFormSku(scope.row.skuId)">{{ $t("message.Button.del") }}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-form-item>
