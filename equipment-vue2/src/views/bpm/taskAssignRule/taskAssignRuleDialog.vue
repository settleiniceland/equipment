  ...
  <div>
    ...
    <el-dialog title="任务分配规则" :visible.sync="visible" width="800px" append-to-body>
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column v-if="modelId" :label='$t("message.Button.operation")' align="center" width="80" fixed="right">
          <template v-slot="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdateTaskAssignRule(scope.row)"
                       v-hasPermi="['bpm:task-assign-rule:update']">{{ $t("message.Button.update") }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
    ...
    <el-dialog title="修改任务规则" :visible.sync="open" width="500px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitAssignRuleForm">{{ $t("message.Button.determine") }}</el-button>
        <el-button @click="cancelAssignRuleForm">{{ $t("message.Button.cancel") }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
