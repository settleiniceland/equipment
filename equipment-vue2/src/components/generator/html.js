...
export function dialogWrapper(str) {
  return `<el-dialog v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="Dialog Titile">
    ${str}
    <div slot="footer">
      <el-button @click="close">{{ $t("message.Button.cancel") }}</el-button>
      <el-button type="primary" @click="handelConfirm">{{ $t("message.Button.determine") }}</el-button>
    </div>
  </el-dialog>`
}
...
function buildFromBtns(scheme, type) {
  ...
  if (scheme.formBtns && type === 'file') {
    str = `<el-form-item size="large">
          ...
          <el-button @click="resetForm">{{ $t("message.Button.origin") }}</el-button>
        </el-form-item>`
    if (someSpanIsNot24) {
      str = `<el-col :span="24">
          ${str}
