<template>
  <div class="panel-tab__content">
    ...
    <el-drawer :visible.sync="fieldModelVisible" title="字段配置" :size="`${width}px`" append-to-body destroy-on-close>
      ...
      <!-- 底部按钮 -->
      <div class="element-drawer__button">
        <el-button size="mini">{{ $t("message.Button.cancel") }}</el-button>
    ...
    <el-dialog :visible.sync="fieldOptionModelVisible" :title="optionModelTitle" width="600px" append-to-body destroy-on-close>
      ...
      <template slot="footer">
        <el-button size="mini" @click="fieldOptionModelVisible = false">{{ $t("message.Button.cancel") }}</el-button>
        <el-button size="mini" type="primary" @click="saveFieldOption">{{ $t("message.Button.determine") }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
