<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      ...
      <el-table-column :label='$t("message.Button.operation")' width="90px">
    ...
    <el-drawer :visible.sync="listenerFormModelVisible" title="执行监听器" :size="`${width}px`" append-to-body destroy-on-close>
      ...
      <el-table :data="fieldsListOfListener" size="mini" max-height="240" border fit style="flex: none">
        ...
        <el-table-column :label='$t("message.Button.operation")' width="100px">
          <template v-slot="{ row, $index }">
      ...
      <div class="element-drawer__button">
        <el-button size="mini" @click="listenerFormModelVisible = false">{{ $t("message.Button.cancel") }}</el-button>
    ...
    <el-dialog title="字段配置" :visible.sync="listenerFieldFormModelVisible" width="600px" append-to-body destroy-on-close>
      ...
      <template slot="footer">
        <el-button size="mini" @click="listenerFieldFormModelVisible = false">{{ $t("message.Button.cancel") }}</el-button>
        <el-button size="mini" type="primary" @click="saveListenerFiled">{{ $t("message.Button.determine") }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
